...
const createEmployerApplicationValidationSchema = zod_1.z.object({
    body: zod_1.z.object({
        ...
        countryOfRegistration: zod_1.z.string({ error: 'Country of Registration is required.' }),
        headquartersAddress: zod_1.z.string({ error: 'Headquarters Address is required.' }),
        ...
        numberOfEmployeesToEnroll: zod_1.z.number({ error: 'Number of employees is required.' }).min(1),
        ...
        eligibilityCriteria: zod_1.z.string({ error: 'Eligibility criteria is required.' }),
        ...
        // ================= Reporting & Analytics =================
        dashboardAccess: zod_1.z.enum([
            'HR Admin only',
            'Multiple users (specify number)',
        ], {
        ...
        communicationSupportNeeded: zod_1.z.enum(['Employee announcement templates', 'Benefits presentation', 'FAQ documents', 'Onboarding webinar'], {
...
const updateEmployerApplicationValidationSchema = zod_1.z.object({
    body: zod_1.z.object({
        ...
        registrationNumber: zod_1.z.string({ error: 'Registration Number is required.' }).optional(),
        countryOfRegistration: zod_1.z.string({ error: 'Country of Registration is required.' }).optional(),
        headquartersAddress: zod_1.z.string({ error: 'Headquarters Address is required.' }).optional(),
        website: zod_1.z.string().url().optional(),
        // ================= Primary Contact =================
        primaryContact: zod_1.z.object({
            fullName: zod_1.z.string({ error: 'Full Name is required.' }).optional(),
            titleOrPosition: zod_1.z.string({ error: 'Title/Position is required.' }).optional(),
            email: zod_1.z.string({ error: 'Email is required.' }).email('Invalid email address').optional(),
            phoneNumber: zod_1.z.string({ error: 'Phone Number is required.' }).optional(),
            whatsappNumber: zod_1.z.string().optional(),
        }).optional(),
        // ================= Employee Coverage Details =================
        numberOfEmployeesToEnroll: zod_1.z.number({ error: 'Number of employees is required.' }).min(1).optional(),
        employeeLocations: zod_1.z.string({ error: 'Employee Location is required.' }).optional(),
        coverageType: zod_1.z.enum(['Employee Only', 'Employee + Spouse', 'Employee + Family'], {
            error: 'Coverage Type is required.',
        }).optional(),
        eligibilityCriteria: zod_1.z.string({ error: 'Eligibility criteria is required.' }).optional(),
        eligibilityCriteriaOther: zod_1.z.string().optional(),
        // ================= Membership Configuration =================
        ...
        // ================= Implementation Timeline =================
        desiredStartDate: zod_1.z.string({ error: 'Desired start date is required.' }).optional(),
        openEnrollmentPeriod: zod_1.z.object({
            start: zod_1.z.string({ error: 'Enrollment start date is required.' }).optional(),
            end: zod_1.z.string({ error: 'Enrollment end date is required.' }).optional(),
        }).optional(),
        communicationSupportNeeded: zod_1.z.enum(['Employee announcement templates', 'Benefits presentation', 'FAQ documents', 'Onboarding webinar'], {
            error: 'Communication Support Needed is required.',
        }).optional(),
