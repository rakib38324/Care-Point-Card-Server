...
const createPaymentValidationSchema = zod_1.z.object({
    body: zod_1.z.object({
        packageStartDate: zod_1.z.string({
            error: 'Package start date is required',
        }),
        packageExpireDate: zod_1.z.string({
            error: 'Package expire date is required',
        }),
        packageType: zod_1.z.string({ error: 'packageType is required' }),
        email: zod_1.z.string().email('Invalid email address'),
        name: zod_1.z.string({ error: 'Name is required' }),
        phone_number: zod_1.z.string({ error: 'Phone number is required' }),
        charged_amount: zod_1.z.string(),
        currency: zod_1.z.string({ error: 'Currency is required' }),
        transaction_id: zod_1.z.string({ error: 'Transaction ID is required' }),
        tx_ref: zod_1.z.string({ error: 'Transaction reference is required' }),
